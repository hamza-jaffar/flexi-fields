...
const EditCustomFieldPage = ({ shop, customField, allFields }: Props) => {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        has_price_addon: !!customField.has_price_addon,
        price: customField.price || '0.00',
        is_active: !!customField.is_active,
        sort_order: customField.sort_order || 0,
        target: customField.target || 'product',
        target_ids: customField.targets ? customField.targets.map((t: any) => t.target_id) : [],
    ...
    const handleSubmit = () => {
        put(customFieldRoutes.update({ customField: customField.id }).url);
    };
                    ...
                    {data.type === 'select' && (
                        <select className="w-full p-2 border rounded">
                            {data.options && data.options.length > 0 ? (
                                data.options.map((opt: any, i: number) => <option key={i} value={opt.value}>{opt.label}</option>)
                            ) : (
    ...
    return (
        <ShopifyLayout>
            ...
            <Page
                ...
                title={`Edit Field: ${customField.name}`}
                primaryAction={{
